...
It is important to understand that when using some agile methodology such as Kanban, there will always be some level of modifications to suit a project. For example in this project the complexity parameters and more thorough analyzing of the cards were skipped in favor of timeboxing, as described in \fullref{sec:timeboxing}. A \gls{wip} limit, according to the Kanban practice \fullref{sec:wip} was never strictly set because it felt excessive when using one developer. It was simply assumed that the developers current work was always on the cards within In Development and the developer alone chose when to pull cards to that column.
