...
Using the values stated by The Agile Manifesto, 12 principles were created that act as the characteristics for agile practices \cite{key1}. You can interpret these principles as the next small abstraction step from The Agile Manifesto where you get a clearer explanation of what agile development is. These are the principles:
...
The methodologies can be vast in their documentation and this report does not seek to explain each methodology in their deepest details. That would simply require too much work and be overwhelming for the readers. For the interested reader, each methodology will have recommended further reading in its introduction.
...
The process policies are rules of the Kanban board. It can state the type and amount of data that each card should contain. It explains the flow of cards traveling across the board. It can contain default implementation details about different classes of cards. It is basically the defined structure of the Kanban board and its cards.
...
\chapter{Theoretical Evaluation}
\label{sec:method_teori}
This chapter aims to compare the four agile methodologies to each other with focus on small development teams. There is no exact definition of what a "small project" or "small team" means. For the sake of clarification we assume that a small project consists of one to five person(s). They can either all be part of the development team or one or two have other responsibilities such as being a costumer, project leader or product owner. We further assume that the small team is a newly founded IT company and not some sort of small project team within a larger company. For example, it could be a handful of friends starting an IT consulting business after getting their master's degree in computer science. So whenever a small company, small project, small team etcetera is mentioned from here on, this is the setup you should have in mind.

\section{Methodology Walktrough}
\subsection{The Agile Principles for Small Teams}
...
Everything in Scrum goes together pretty neatly as seen in Figure \ref{fig:scrum}. Each practice is strongly connected to several of the others. Within the events, all three roles work together tightly and always have the artifacts in focus. Generally this is a good thing since it shows coherence and makes it easier to grasp the meaning of each practice. However to be able to include all practices one would at least need three people to fill the roles, which would mean a Development Team of size one to three for small companies. Another solution would be to have everyone part of the Development Team where two of these have extended roles as a Product Owner and Scrum Master. It is certainly possible to include all practices for a small project but it would also most certainly lead to lots of overhead work when so few people are involved.
...
\gls{xp} as a whole is quite clearly too vast for small newly founded IT companies. But they do have many stand-alone practices that would be easy to start with and then slowly built upon, practice by practice. For example, all the quality ensuring practices mentioned. However, how agile would it be to have a bunch of practices satisfying only one agile principle? To ensure that the other parts of the agile principles are included one would have to look into the Planning Game. But instead of diving into that vast practice it would make more sense starting with Scrum where the different parts of The Planning Game are more clearly distinguished. 
...
Since it has this high abstraction level it is hard to say which agile principles are touched. However the practices line of argument means that the more social principles 4, 5, 6 and 8 are being satisfied or are at least are in focus.
...
\section{Chosen Methodology for the Project}
Kanban became the chosen methodology for the project, see \fullref{sec:project_description}. There are several reasons for the methodology to be favorable for the project. The methodology covers most of the agile principles but is extremely lightweight compared to Scrum and especially \gls{xp}. For a small project this is essential since the smaller the project team the larger the overhead for a methodology. And in this particular project there will be just one developer and the owner of the project. The fact that Kanban is meant to be expanded on demand and that methodologies like Scrum-ban exists also means that it should be easy to let the methodology grow as the project/company grows.
...
Regarding Crystal Clear, the focus on social environment makes it a pretty poor choice when working solo as a developer. Even if you are a handful of developers it can be hard to practically follow such abstract ideas. The methodology also does not have the same connection for its practices as the other methodologies.

\chapter{Project}
This chapter goes through the project and specifically the workflow of the project that was and, at this time, is still being developed using Kanban. The chapter tries to keep a natural timeline but not to a full extent since many logical components of the development process were themselves developed over time. Figure \ref{fig:agile_waterfall} describes the problem pretty well. In other words, each component individually, like creating the Process Policies, follow a logical timeline.

Since the project is a private initiative by the product owner, the developer works from his home. Tuesday at 10:00 was initially selected as a weekly meeting time to ensure that the developer and product owner regularly see each other face to face. During the course of the project extra meeting times were added, especially at the start of the project to get the ball rolling. 
...
\section{Using Kanban}

\subsection{Process Policies}
The first thing that was looked into, after deciding on using Kanban, was the layout of the Kanban cards, in other words the Process Polices. Typically these cards are either simply tasks and/or \glspl{us}. Since Kanban itself does not explicitly state the layout of the cards, the strategies described in Use-Case 2.0 were selected to form the basic structure \cite{key25}. This will be described further in \fullref{sec:use_case}.

Most of the policies are written in \appfullref{app:project_process_policies} that is a mix of process policies connected to Use-Case 2.0 and the Kanban cards. 

The Process Policies were not all written at one point in time. As the project grew more policies were added. Using


\begin{figure}[h]
  \centering
    \includegraphics[width=0.8\textwidth]{img/process_policy_sample.png}
...
Use-Case 2.0 is a popular technique meant to be lightweight, scalable, versatile and easy to use \cite{key25}. If it delivers what it promises it sure fits the description of what you look for in a small project with few members. Also the technique is meant to be used with methodologies such as Kanban and Scrum. Without going into to much detail the technique, when used with Kanban, can be summarized as following:

\begin{enumerate}
\item Agree on the goals and scope of the system and identify all actors and ways of using the system. Repeat this every now and then as things can change as the project develops.
\item Each use-case represents a meaningful logical interaction between some actor and the system.
\item Create a narrative for each use-case that explains the actors goal and interaction on an abstract level and in everyday language, similar to how \gls{us} are written.
...
\item When development on a use case grows "near", create slices for the use-case. As the name suggests, try to create each slice so they correspond to a suitable and somewhat equally sized work. The slices are created by either splitting a use-case flow or combining multiple flows. The slice can also go deeper in detail if wanted or needed, both in terms of the use-case flow steps and test cases.
...
\includegraphics[width=0.43\textwidth]{img/use_case_card.png}
\vspace{-5pt}
  \caption{Example of a Use-Case Flow Card.}
   \label{fig:use_case_card}
   \vspace{5pt}
   
   \includegraphics[width=0.43\textwidth]{img/use_case_flow_card.png}  
   \vspace{-5pt}
   \caption{Example of a Use-Case Flow Card.}
  \label{fig:use_case_flow_card}
   \vspace{5pt}

   \includegraphics[width=0.43\textwidth]{img/task_card.png}
   \vspace{-5pt}
...
Everything about the project was written inside the cards. Every task or idea had a card connected to it. The cards has basically three movement types after creation: It can move its way to the right on the board until it ends up in Deployed and Live. It can at some point in some category be removed, or archived as it is called in Trello, if it is found unwanted. These cards are not permanently deleted and can always be found again after removal. Finally a card can find itself to be frozen in some category, except in In Development and in Done. The cards in Deployed and Live are for instance normally frozen in place. Other examples are low priority cards in the Backlog or earlier categories that have a small value but high complexity.  

During development new cards were continuously created and added to the board. At each meeting, more cards could be created and added. It was during the meetings that it was decided which cards should be kept or removed. Also the cards in the Done column was tested and discussed together with the cards in the In Development column. Another important part of the meetings were organizing the Backlog column so that the cards were kept arranged in priority order from top to bottom.

The Report cards are special. They were added early since the report writer is the developer. Therefor it made sense including these cards since it gives the product owner a better overview of what is going on at every meeting. After all, one of the practices in Kanban is about having transparency, see \fullref{sec:kanban_visualize}.
...
\section{Developer \& Product Owner Experience}
